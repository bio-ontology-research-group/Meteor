...
PRED=(sys.argv[sys.argv.index('--pred')+1] if '--pred' in sys.argv else 'dpz')
V8=f'{B}/meteor_v8_evw_p2mu3_run/meteor_out/{PRED}_vanilla'; V7=f'{B}/meteor_v7_run/meteor_out/{PRED}_vanilla'
GEMDIR=external_path('curated_gems'); RECON=f'{B}/meteor_v8/results/toolcompare/recon_models'
...
rows=[]
ARMS=['v8(METEOR)','baseline']
print('=== B3 predictor-generalization ('') 3-arm (v7 / v8-evw / threshold+gapfill / Reconstructor-dpz) ===')
...
json.dump(rows,open(f'{OUT}/b3_{PRED}.json','w'),indent=1)
def mn(a,k):
...
print('-> results/toolcompare/b3_{PRED}.json')
