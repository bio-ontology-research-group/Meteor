...
def lookup(df,pid,nid=None):
    ...
    return None
def rf(gt,sc,tau):
    pos={ec for ec,s in sc.items() if s>=tau}; tp=len(gt&pos)
...
for b in ['clean','dpz','enzbert','graphec','mapred','topec']:
    bc={}; tc={}; n=0
    RB={t:0.0 for t in TAUS}; FB={t:0.0 for t in TAUS}; RT={t:0.0 for t in TAUS}; FT={t:0.0 for t in TAUS}
    for ent,gt in gt_ecs.items():
        ...
        n+=1
        for t in TAUS:
            rb,fb=rf(gt,sb,t); rt,ft=rf(gt,st,t); RB[t]+=rb; FB[t]+=fb; RT[t]+=rt; FT[t]+=ft
    if n:
        print(f'=== {b} (n={n}) ===')
        print('  tau  |  Recall base->top1     |  F1 base->top1')
        for t in TAUS:
            print('  %.1f  |  %.3f -> %.3f (%+.3f)  |  %.3f -> %.3f (%+.3f)'%(t,RB[t]/n,RT[t]/n,(RT[t]-RB[t])/n,FB[t]/n,FT[t]/n,(FT[t]-FB[t])/n))
